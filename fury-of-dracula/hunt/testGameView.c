...
int main(void)
{
	...
	{///////////////////////////////////////////////////////////////////
		...
		{	// Testing get last moves function
			// Regular Case
			int numMoves = 0; bool canFree = false;
			...
			// Too many moves case
			moves = GvGetLastMoves(gv, PLAYER_LORD_GODALMING, 10,
			                                  &numMoves, &canFree);
			assert(numMoves == 7);
			assert(moves[0] == LISBON);
			assert(moves[1] == CADIZ);
			...
			printf("Get Last Moves tests passed\n");
		}
		...
		{
			int numLocs = 0; bool canFree = false;
			PlaceId *locs = GvGetLastLocations(gv, PLAYER_DRACULA, 4,
			                                      &numLocs, &canFree);
			assert(numLocs == 4);
			...
			locs = GvGetLastLocations(gv, PLAYER_DRACULA, 10,
			                                     &numLocs, &canFree);
			assert(numLocs == 6);
			assert(locs[0] == STRASBOURG);
			assert(locs[1] == CITY_UNKNOWN);
			...
			printf("Get last locations tests passed\n");
		}
	...
	{//////////////////////////////////////////////////////////////////////
	    ...
	    {
	        printf("Test for Hunter Resting Healing and Dracula Healing at Castle Dracula\n");

		    char *trail =
			    "GST.... SAO.... HCD.... MAO.... DGE.V.. "
			    "GGEVD.. SAO.... HCD.... MAO.... DCD.... "
			    "GGE.... ";
	    ...
	    {
            printf("Test for No Health Cap Dracula\n");
	    ...
	    { // THIS TEST IS DUMB UNLESS WE KNOW WHAT HAPPENS WHEN THE GAME ENDS - NOT FINISHED
	      // So far test shows drac heath goes to zero, but doesnt assert that the game ends cos drac dies  
	        printf("Test for Dracula dying\n");
		    ...
		    assert(GvGetHealth(gv, PLAYER_DRACULA) ==
				    GAME_START_BLOOD_POINTS - 4*(LIFE_LOSS_HUNTER_ENCOUNTER));
		    assert(GvGetHealth(gv, PLAYER_LORD_GODALMING) ==
				GAME_START_HUNTER_LIFE_POINTS - LIFE_LOSS_DRACULA_ENCOUNTER);
			assert(GvGetHealth(gv, PLAYER_VAN_HELSING) ==
				GAME_START_HUNTER_LIFE_POINTS - LIFE_LOSS_DRACULA_ENCOUNTER);
	    ...
	    {/* I'm not sure how to implement the trails part
            printf("Test for no encounter at sea\n");
		    ...
		    assert(GvGetHealth(gv, PLAYER_DRACULA) ==
				    GAME_START_BLOOD_POINTS - (2 * LIFE_LOSS_SEA));
		    assert(GvGetPlayerLocation(gv, PLAYER_DRACULA) == ATLANTIC_OCEAN);
            assert(gv->trail[1].enc == NO_ENCOUNTER);
	    ...
	    {
	        printf("Testing a hunter death resulting in point loss and health gain back(revive)\n");
	    ...
	    {
	        printf("Testing Dracula has no rail connections\n");
	    ...
	    {
	        printf("Testing Dracula has no rail connections 2\n");
	    ...
	    {
	        printf("Testing Dracula doubling back (2)\n");
	    ...
	    {
	        printf("Testing Dracula doubling back (3)\n");
	    ...
	    {
	        printf("Testing Dracula doubling back (4)\n");
	    ...
	    {
	        printf("Testing Dracula doubling back (5)\n");
	    ...
	    {
	        printf("Test that Dracula cant move to Hospital\n");
