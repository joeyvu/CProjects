...
Map HvGetMap(HunterView hv) {
	return getMap(hv->gv);
}

PlaceId *HvPossibleLocs(HunterView hv, PlaceId src, int *size, Round startRound, Round endRound, Player player) {
	return possibleLocs(hv -> gv, src, size, startRound, endRound, player);
}

PlaceId *HvShortestPath(HunterView hv, PlaceId src, PlaceId dest, int *size, Round round, Player player){
	return shortestPath(hv -> gv, src, dest, size, round, player);
}