...
#define MAXLINE 1000
#define MAXWORD 100
#define STARTING "*** START OF" // used to begin file scanning
#define ENDING   "*** END OF" // used to end file scanning
...
int buildWordFreqDict(Dict wfreqs, FILE *in, Dict stopwords) {
   ...
   while (fgets(buffer, MAXLINE, in) != NULL &&
      ...
      while (sscanf(buffer + offset, "%s%n", word, &charsRead) == 1) {
         ...
         offset += charsRead; //iterates
      } 
   }
   return EXIT_SUCCESS;
}

// removes invalid characters before inputting into dictionary
void removeInvalidChars(char *buffer) {
...
// converts input words into lower case for inputting into.. 
// ..dictionary
void convertToLowercase(char *word) {
